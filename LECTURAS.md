...
|--------|--------|------|--------|------|
| 1 | 0 | §1.1–1.2 | — | Qué es tiempo real; las historias del Shuttle y del Patriot son el argumento del curso |
| 2 | 0 | §1.3 | — | Fuentes de impredecibilidad: DMA, caché, interrupciones, syscalls |
| 3 | 1 | §2.1–2.3 | §2.4 (se retoma en la semana 10) | Modelos de tarea y tipos de restricción — el vocabulario del semestre |
| 4 | 1 | §1.3 (releer con ojos de medición), §13.5.1 | — | El libro es corto en *cómo* medir; eso lo cubre el laboratorio |
| 5 | 2 | §10.1–10.3 | §10.4–10.5 (detalle de implementación) | Anatomía de un kernel RT: estados, colas de listos, cambio de contexto — Zephyr como ejemplo vivo |
| 6 | 2 | §10.6–10.7 | Implementación de CABs (§10.6.2) | Comparar los CABs con las message queues de Zephyr; §10.7: el overhead del kernel existe y se mide |
| 7 | 3 | §4.1–4.4 (enunciados, tablas y ejemplos), §4.7; enunciado de optimalidad de EDF (§3.3.1) | Las derivaciones de U_lub (§4.3.2–4.3.3, ~7 páginas de cálculo), demostraciones de Teo. 4.1 y 4.2; el resto del cap. 3 | El ejemplo de la Fig. 4.13 (RM falla, EDF no, con U = 0.97) se reproduce en el lab sobre la S3 |
| 8 | 3 | §4.5.1–4.5.2 (RTA); §7.1–7.3, reglas de PIP (§7.6.1) y PCP (§7.7.1), enunciado del Teo. 7.2 | §4.5.3–4.5.4; cadena de lemas de §7.6.2, cómputo exacto de bloqueo (§7.6.3), §7.8 (SRP) | La RTA es un algoritmo de 15 líneas (Fig. 4.17), no un teorema: en el lab se implementa en Python con los C_i medidos. **Parcial: caps. 2, 4 y 7** — el cap. 14 trae soluciones de los ejercicios de los caps. 1–9 para practicar |
...
| 10 | 5 | §13.5 (herramientas de WCET y análisis); §2.4 | — | El libro no tiene capítulo de multiprocesador — las anomalías de §2.4 son el gancho teórico; el resto es material del curso |
...
| 13 | 7 | Cap. 11: §11.2 (de requisitos a restricciones de tiempo), §11.3, §11.5.2 (criticidad mixta) | — | La tabla de tareas del robot en §11.4 (períodos, WCETs, duro/suave) es el espejo del task set del SoilSense Control |
